...
interface FeaturesProps {
  features: Array<{
    ...
    iconVariant?:
      "default" | "accent" | "muted" | "success" | "warning" | "error";
  }>;
  title?: string;
  subtitle?: string;
  columns?: 1 | 2 | 3 | 4;
...
function ContactForm() {
  // The generic is what types `register` and the submitted values. Without it
  // both fall back to `FieldValues` and a typed handler no longer fits.
