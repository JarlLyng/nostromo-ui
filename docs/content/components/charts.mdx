...
<LiveCode code={`import { Chart } from '@jarllyng/nostromo'
...
{ month: 'Apr', desktop: 180, mobile: 130, target: 150 },
]

export default function StackedWithTarget() {
  return (
    <ChartContainer data={data} height={320} ariaLabel="Visitors by device, against target">
      <ChartGrid />
...
{ month: 'Feb', latency: 240 },
{ month: 'Mar', latency: 210 },
{ month: 'Apr', latency: 320 },
]

export default function LatencyBudget() {
  return (
    <ChartContainer data={data} height={280} ariaLabel="Latency against budget">
      <ChartGrid />
      <ChartXAxis dataKey="month" />
      <ChartYAxis />
      <ChartTooltip />
      <ChartArea dataKey="latency" />
      <ChartReferenceLine y={250} label="Budget" />
    </ChartContainer>
  )
}`} />
