...
describe("Contrast Accessibility Tests", () => {
  ...
  describe("Theme Color Contrast (Light Mode)", () => {
    ...
    const background =
      extractHSLValue(themeContent, "background") || "0 0% 98%";
    const mutedForeground =
      extractHSLValue(themeContent, "muted-foreground") || "0 0% 32%";
    const muted = extractHSLValue(themeContent, "muted") || "0 0% 96%";
    const cardForeground =
      extractHSLValue(themeContent, "card-foreground") || "0 0% 9%";
    const card = extractHSLValue(themeContent, "card") || "0 0% 98%";
    const popoverForeground =
      extractHSLValue(themeContent, "popover-foreground") || "0 0% 9%";
    const popover = extractHSLValue(themeContent, "popover") || "0 0% 98%";
    const primaryForeground =
      extractHSLValue(themeContent, "primary-foreground") || "0 0% 98%";
    const primary = extractHSLValue(themeContent, "primary") || "195 100% 50%";
    const secondaryForeground =
      extractHSLValue(themeContent, "secondary-foreground") || "0 0% 9%";
