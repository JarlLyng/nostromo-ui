...
describe("Tooltip", () => {
  ...
  it("applies correct variant classes", () => {
    ...
    expect(screen.getByText("Test tooltip").closest("div")).toHaveClass(
      "bg-success",
  ...
  it("applies custom className", () => {
    render(
      <Tooltip content="Test tooltip" open onOpenChange={() => {}}>
        ...
        <TooltipContent className="custom-tooltip">Test tooltip</TooltipContent>
      </Tooltip>,
    );

    // Brug getAllByRole og filtrer på custom-tooltip class
    const tooltips = screen.getAllByRole("tooltip");
    const customTooltip = tooltips.find((el) =>
...
describe("TooltipContent", () => {
  ...
  it("renders arrow with correct classes", () => {
    ...
    const arrow = screen
      ...
      .parentElement?.querySelector(".w-2.h-2.rotate-45");
...
describe("TooltipContent visibility", () => {
  /**
   * TooltipContent never read the open state. `data-open` was written on the
   * wrapper and nothing acted on it, so the tooltip was permanently visible -
   * sitting on top of its own trigger on every page that used one.
   */
  it("is hidden while the tooltip is closed", () => {
    render(
    ...
    const tip = screen.getByText("Tip text").closest('[role="tooltip"]')!;
  ...
  it("is shown once the tooltip opens", () => {
    render(
      <Tooltip content="ignored" open>
        <TooltipTrigger>Hover me</TooltipTrigger>
        <TooltipContent>Tip text</TooltipContent>
      </Tooltip>,
    );
