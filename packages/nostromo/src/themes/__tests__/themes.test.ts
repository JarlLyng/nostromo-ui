...
describe("Theme CSS Files", () => {
  ...
  themes.forEach((theme) => {
    ...
    it(`should have ${theme} theme semantic color tokens`, () => {
      const themePath = join(themesDir, `${theme}.css`);
      const css = readFileSync(themePath, "utf-8");
      // Tokens live in the private --nostromo-* namespace; --color-* is owned by
      // Tailwind v4 and reserving it here would create a circular reference.
      expect(css).toContain("--nostromo-color-background");
      expect(css).toContain("--nostromo-color-foreground");
      expect(css).toContain("--nostromo-color-primary");
      expect(css).toContain("--nostromo-color-muted");
    });
